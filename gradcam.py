...
class PropBase(object):
    ...
    def backward(self, mu, logvar, mu_avg, logvar_avg):
        ...
        z, eps  = self.model.reparameterize_eval(mu, logvar)
        z       = z.cuda()
        ...
        if flag == 1:
            ...
            #self.score_fc = torch.norm(mu, p=1)
...
class GradCAM(PropBase):
    ...
    def compute_gradient_weights(self):
        ...
        self.weights = nn.AvgPool1d(self.map_size)(self.grads)
    ...
    def generate(self):
        ...
        gcam = F.conv2d(self.activiation, (self.weights.cuda()), padding=0, groups=len(self.weights))
        gcam = gcam.squeeze(dim=0)
        gcam = F.upsample(gcam, self.image_size, mode="linear")
        gcam = torch.abs(gcam)
        print(gcam.size())
        return gcam
